...
#these are the tables of child company
...
#we are only adding 1dec.csv data to landing in s3 to see how staging table works 
...
# DBTITLE 1,Write Delta Table
#we are now writing to our staging table in bronze layer
df.write\
 .format("delta") \
 .option("delta.enableChangeDataFeed", "true") \
 .mode("overwrite") \
...
#here we moved the 1dec.csv file from landing to processed in s3.] 
...
#now we are reading the data from staging in bronze layer
...
#OBVIOUSLY SHOWS OONLY 1 dec dated data ,as it contains only that
...
if not (spark.catalog.tableExists(silver_table)):
    ...
    #in our case here else part will be executed as table exists
...
#now writing to our staging table present in our silver layer
df_joined.write\
 .format("delta") \
 .option("delta.enableChangeDataFeed", "true") \
 .mode("overwrite") \
...
#now we are reading the data from staging in silver layer
...
#as we did transofromation

# COMMAND ----------

# now we push it to gold table what is gold table
gold_table
